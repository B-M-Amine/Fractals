...
public class Mandelbrot extends Task<ArrayList<ImageView>> {
	
	/* Methode qui prend en parametre un Complexe C et un indice maximal d'iterations */
	/* Retourne -1 si la suite suivante est bornée : Z(n+1) = Z(n) + C tel que Z(0) = 0+0i */
	/* Si elle est n'est pas bornée, retourne l'indice a partir duquel elle ne l'est pas*/
	public static int converge(Complex c, int precision) {
		int index = Complex.divergenceIndex(new Complex(0, 0), c, precision);
	...
	public Mandelbrot(int size, Bornes bornes, int precision, Pane pane) {
		super();
		this.size = size;
		this.bornes = bornes;
	...
	/* Methode de création de l'ensembe de Mandelbrot */
	...
	protected ArrayList<ImageView> call() throws IOException {
		...
		for (i = 0; i < size; i++) {
			for (j = 0; j < size; j++) {
				...
				index = converge(number, precision);
